...
import { product, productOrNot, productOrOne } from "../../../mathlib/ConvenientExpressions";
...
export class PullConstantsFromIntegrals extends NoContextExpressionSimplificationRule {
    protected appliesImpl(exp: Expression): boolean {
        ...
        return i.integrand instanceof Product && i.integrand.factors.some(f => {
            return f.isConstant
    ...
    protected applyImpl(exp: Expression): Set<Argument> {
        ...
        const pulledFactors = productOrNot(...integrand.factors.filter(f => f.isConstant))
        const newIntegrand = productOrOne(...integrand.factors.filter(f => !f.isConstant))

        const result = product(pulledFactors, Integral.of(newIntegrand, i.relativeTo))

        return setOf(new Argument(setOf(exp), {
            n: exp,
            r: Relationship.Equal,
            n1: result
        }, "Pull constants from integral", RULE_ID))
