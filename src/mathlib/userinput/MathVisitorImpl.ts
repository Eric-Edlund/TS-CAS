...
import {
    ...
    OpenContext,
    ClosedAtomContext,
    ClosedIsRight_ClosedContext,
    Right_ClosedIsOpenContext,
    Right_ClosedImplicitProductContext,
    IntegralContext,
    LogContext
...
export class ExpressionVisitor extends arithmeticVisitor<Expression> {
    ...
    visitPower = (ctx: PowerContext): Expression => {
        ...
        return Exponent.of(this.visit(ctx._left), this.visit(ctx._right))
    }
    ...
    visitDivision = (ctx: DivisionContext): Expression => {
        return Fraction.of(this.visit(ctx._left), this.visit(ctx._right))
    }

    visitProduct = (ctx: ProductContext): Expression => {
        return Product.of([...ctx.closed_list().map(exp => this.visit(exp))])
    }
    ...
    visitRight_ClosedImplicitProduct = (
        ...
        //this.printChildren(ctx)
        return Product.of([this.visit(ctx._left), this.visit(ctx._right)])
    }

    visitIntegral = (ctx: IntegralContext): Expression => {
        return Integral.of(this.visit(ctx._integrand), Variable.of("x"))
    }
    ...
    visitUnaryOnExpression = (ctx: UnaryOnExpressionContext): Expression => {
        ...
        if (isPositive) return this.visit(ctx.closed())
    ...
    visitAtom = (ctx: AtomContext): Expression => {
        if (ctx.VARIABLE() != null) {
            ...
            return Integer.of(
                Number.parseFloat(ctx.SCIENTIFIC_NUMBER().getText())
            )
        } else {
    ...
    visitClosedIsRight_Closed = (
        ctx: ClosedIsRight_ClosedContext
    ): Expression => {
