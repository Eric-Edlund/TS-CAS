...
impl DerivationRule for IntegrateArcTrig {
    fn apply(&self, input: Expression) -> Vec<(Expression, Rc<Argument>)> {
        ...
        let Expression::Fraction(f) = integral.integrand()
        else {
            return vec![]
        };
        ...
        let result: Expression = if let Expression::Sum(sum) = f.denominator() {
            ...
            if not.first().unwrap() != &&Exponent::of(
            ...
            let Expression::Sum(sum) = root.base()
            else {
                return vec![]
            };
            let terms = sum.terms();
            let a = if !terms.iter().any(|t| is_constant(t, &integral.relative_to())) {
                return vec![]
            } else {
                terms.iter().find(|t| is_constant(t, &integral.relative_to()))
                    .unwrap()
            };
            ...
            let root = match factors.iter().find_map(|f| match f {
                Expression::Exponent(e) => if e.power() == Fraction::of(i(1), i(2)) {
                    Some(e)
                } else {
                        None
                    },
            ...
            for term in terms {
                if is_constant(term, &integral.relative_to()) {
                    let Expression::Negation(n) = term 
                    else {
                        return vec![]
                    };
                    a = Some(n.child());
                } else {
                    let Expression::Exponent(e) = term
                    else {
                        return vec![]
                    };
...
mod tests {
    ...
    fn test_1() {
        ...
        let start = Integral::of(
            Fraction::of(
                Integer::of(1),
                sqrt(sum_of(&[Integer::of(4), Negation::of(
                    Exponent::of(v("x"), Integer::of(2))
                )]))
            ),
            v("x")
        );
        ...
        assert_eq!(result, product_of(&[i(1), arcsin(Fraction::of(
            v("x"), sqrt(i(4))
        ))]));
        ...
        assert_eq!(result2, product_of(&[
            i(7),
            Fraction::of(i(1), sqrt(i(2))),
            arctan(Fraction::of(v("x"), sqrt(i(2))))
        ]));

        let start3 = Integral::of(Fraction::of(
            v("a"),
            product_of(&[
                v("x"),
                sqrt(sum_of(&[
