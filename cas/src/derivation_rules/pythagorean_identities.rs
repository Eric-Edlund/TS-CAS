...
impl DerivationRule for Pythagoras {
    fn apply(&self, input: Expression) -> Vec<(Expression, Rc<Argument>)> {
        ...
        let squared_trig_terms = sum.terms().iter().filter_map(unwrap_squared_trig)
            .collect::<HashSet<_>>();
        // List of expressions acted on by trig expressions
        let trig_params = squared_trig_terms.iter().map(|x| x.exp())
            .collect::<HashSet<_>>();
        let ones = sum.terms().iter()
            .filter(is_one)
            .count();

        let has_term = |op: TrigFn, exp: &Expression| {
            squared_trig_terms.iter()
                .find(|t| t.operation == op && &t.exp() == exp)
                .is_some()
        };
        ...
        for exp in &trig_params {
            if has_term(TrigFn::Sin, exp) && has_term(TrigFn::Cos, exp) {
                ...
                results.push(sum_of_iter(&mut sum.terms().into_iter()
                    .filter(|term| {
            ...
            if has_term(TrigFn::Cot, exp) && ones > 0 {
                ...
                results.push(sum_of_iter(&mut sum.terms().into_iter()
                    ...
                    }).chain(&[Exponent::of(TrigExp::of(TrigFn::Csc, exp.clone()), Integer::of(2))]).cloned()
                ))
            }
            if has_term(TrigFn::Tan, exp) && ones > 0 {
                ...
                results.push(sum_of_iter(&mut sum.terms().into_iter()
                    ...
                    }).chain(&[Exponent::of(TrigExp::of(TrigFn::Sec, exp.clone()), Integer::of(2))]).cloned()
                ))
            }
        }

        results.into_iter().map(|equiv| {
            (equiv,
...
#[cfg(test)]
mod tests {
    use crate::{convenience_expressions::v, expressions::{sum::sum_of, Exponent}};
    ...
    fn test_1() {
        ...
        assert_eq!(result3, Exponent::of(TrigExp::of(TrigFn::Csc, v("x")), Integer::of(2)));
    }
