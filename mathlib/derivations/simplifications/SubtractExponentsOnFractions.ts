...
import { f, negative, num, productOrNot, sum } from "../../ConvenientExpressions";
...
export class SubtractExponentsOnFractions extends NoContextExpressionSimplificationRule {
    ...
    protected applyImpl(exp: Expression): Set<Argument> {
        ...
        const top = frac.numerator instanceof Product ? frac.numerator.factors : [frac.numerator]
        const bottom = frac.denominator instanceof Product ? frac.denominator.factors : [frac.denominator]
        ...
        topExponents.forEach(e => {
            if (has(bottomExponents.map<Expression>(e => e.base), e.base)) commonBases.add(e.base)
        })
        ...
        if (uneffectedBottomFactors.length == 0) uneffectedBottomFactors.push(num(1))
